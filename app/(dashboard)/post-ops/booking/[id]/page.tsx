            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                ...
                {flights.length > 0 && (
                    <div className="rounded-2xl p-5" style={{ background: '#FFFFFF', border: '1px solid rgba(5,34,16,0.08)', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
                        ...
                        {flights.map((f: any) => (
                            <div key={f.id} className="flex justify-between py-2" style={{ borderBottom: '1px solid rgba(6,161,92,0.05)' }}>
                                <div>
...
{["single_choice", "Single Choice"].includes(item.type) && (item.options || item.points) && (<div className="mt-3 space-y-2">{(item.options || item.points || []).map((opt, i) => (<label key={i} className="flex items-center gap-2 cursor-pointer"><input type="radio" disabled={item.checked} name={"single-"+item.id} checked={item.response === opt} onChange={() => updateSalesItemState(item.id, { response: opt })} className="w-3.5 h-3.5" style={{ accentColor: "#06a15c" }} /><span className="font-sans text-xs" style={{ color: "rgba(5,34,16,0.8)" }}>{opt}</span></label>))}</div>)}
{["multiple_choice", "Multiple Choice", "multiple_select"].includes(item.type) && (item.options || item.points) && (<div className="mt-3 space-y-2">{(item.options || item.points || []).map((opt, i) => { const currentArr = Array.isArray(item.response) ? item.response : []; return (<label key={i} className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={currentArr.includes(opt)} onChange={(e) => { let n = [...currentArr]; if (e.target.checked) n.push(opt); else n = n.filter(x => x !== opt); updateSalesItemState(item.id, { response: n }); }} className="w-3.5 h-3.5 rounded border-gray-300 text-emerald-600 focus:ring-emerald-600" /><span className="font-sans text-xs" style={{ color: "rgba(5,34,16,0.8)" }}>{opt}</span></label>) })}</div>)}
{["rating", "Rating", "rating_5", "rating_10"].includes(item.type) && (<div className="mt-3 flex items-center gap-1.5">{Array.from({ length: item.type === "rating_10" ? 10 : (parseInt(item.extraInfo || "5") || 5) }).map((_, i) => (<button key={i} onClick={() => updateSalesItemState(item.id, { response: (i + 1).toString() })} className="text-xl transition-colors hover:scale-110" style={{ color: (parseInt(item.response || "0")) > i ? "#f59e0b" : "rgba(5,34,16,0.1)" }}>?</button>))} <span className="ml-2 font-sans text-[10px] text-gray-400">({item.response || 0} / {item.type === "rating_10" ? 10 : (parseInt(item.extraInfo || "5") || 5)})</span></div>)}
